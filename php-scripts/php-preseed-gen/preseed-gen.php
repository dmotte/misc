...
$data['disk'] ??= '';
if (str_ends_with($data['disk'], '!')) {
    $data['disk-confirm'] = true;
    $data['disk'] = substr($data['disk'], 0, -1);
} else $data['disk-confirm'] = false;
ensure_value_ok(
    'disk',
    $data['disk'],
    fn($x) => preg_match('/\A[0-9A-Za-z\/_-]*\z/', $x) === 1,
...
if (
    ...
    if ($data['sudo-nopasswd']) {
        ...
        ' ALL=(ALL:ALL) NOPASSWD: ALL") \\', PHP_EOL;
        echo '        /etc/sudoers.d/50_', $data['username'], '_nopasswd; \\',
        PHP_EOL;
    }
