...
def tree_snapshot(rinv: ResticInvoker, snapshot_id: str) -> Iterator[str]:
    ...
    for entry in rinv.restic_json(['ls', snapshot_id]):
        if not all(k in entry for k in ('type', 'path', 'mode', 'mtime')):
            continue

        entry['path'] = entry['path'].removeprefix('/')
        entry['mode'] = oct(stat.S_IMODE(entry['mode']))[2:]

        if entry['type'] == 'dir':
            yield f'{entry['path']}/;{entry['mode']};-1;DIR\n'
            continue

        entry['size'] = str(entry['size'])
        entry['mtime'] = str(int(dt.fromisoformat(entry['mtime']).timestamp()))

        yield f'{entry['path']};{entry['mode']};{entry['size']};{entry['mtime']}\n'
...
def tree_local(root_path: str, excludes: list[str] = [],
    ...
    for entry in entries:
        ...
        mode_str = oct(stat.S_IMODE(st.st_mode))[2:]

        if stat.S_ISDIR(st.st_mode):
            yield f'{rel_path}/;{mode_str};-1;DIR\n'
            yield from tree_local(os.path.join(root_path, entry.name),
                                  excludes, f'{rel_path}/')
        else:
            yield f'{rel_path};{mode_str};{st.st_size};{int(st.st_mtime)}\n'
