...
archive_url=${STANDALONE_VSCODE_ARCHIVE_URL:-https://code.visualstudio.com/sha/download?build=stable&os=linux-x64}
...
if [ -d "$main_dir" ]; then
    ...
    "$main_dir/VSCode-linux-x64/bin/code" -v
...
Exec=$main_dir/VSCode-linux-x64/code %f
Comment=Visual Studio Code
Terminal=false
Icon=$main_dir/VSCode-linux-x64/resources/app/resources/linux/code.png
Type=Application
EOF
fi
