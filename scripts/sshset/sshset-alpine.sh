...
if [ "$EUID" = 0 ]; then
    if [ "$setup_server" = true ]; then
        ...
        if [ "$setup_server_rc" = true ]; then
            ...
            if [ -n "$files" ]; then
                ...
                # Note: not using install's "-T" flag as it's
                # not supported in Alpine
                echo "$content" | install -vm644 /dev/stdin /etc/ssh/sshrc
            fi
        fi
    fi
    ...
    if [ "$setup_client" = true ]; then
        ...
        if [ -n "$files" ]; then
            ...
            # not supported in Alpine
            echo "$content" |
                install -vm644 /dev/stdin /etc/ssh/ssh_known_hosts
        fi
    fi
    ...
    while IFS= read -r user || [ -n "$user" ]; do
        ...
        if [ "$setup_server" = true ]; then
            ...
            if [ -n "$files" ]; then
                ...
                # Note: not using install's "-T" flag as it's
                # not supported in Alpine
                echo "$content" | install -o"$user" -g"$user_group" -vm600 \
                ...
                # Note: not using install's "-T" flag as it's
                # not supported in Alpine
                install -o"$user" -g"$user_group" -vm600 \
                    "$user_dir/authorized-keys/id_ed25519.pub" \
            ...
            if [ "$setup_server_rc" = true ]; then
                ...
                if [ -n "$files" ]; then
                    ...
                    echo "$content" | install -o"$user" -g"$user_group" \
                        -vm600 /dev/stdin "$user_home/.ssh/rc"
                fi
            fi
        fi
        ...
        if [ "$setup_client" = true ]; then
            ...
            if [ -n "$files" ]; then
                ...
                # Note: not using install's "-T" flag as it's
                # not supported in Alpine
                echo "$content" | install -o"$user" -g"$user_group" -vm644 \
    ...
    if [ "$setup_server" = true ]; then
        ...
        if [ -n "$files" ]; then
            ...
            # Note: not using install's "-T" flag as it's
            # not supported in Alpine
            echo "$content" | install -vm600 /dev/stdin ~/.ssh/authorized_keys
        elif [ "$gen_authkey" = true ]; then
            ...
            # Note: not using install's "-T" flag as it's
            # not supported in Alpine
            install -vm600 "$data_dir/authorized-keys/id_ed25519.pub" \
                ~/.ssh/authorized_keys
        fi
        ...
        if [ "$setup_server_rc" = true ]; then
            ...
            if [ -n "$files" ]; then
                ...
                # Note: not using install's "-T" flag as it's
                # not supported in Alpine
                echo "$content" | install -vm600 /dev/stdin ~/.ssh/rc
            fi
        fi
    fi
    ...
    if [ "$setup_client" = true ]; then
        ...
        if [ -n "$files" ]; then
            ...
            # Note: not using install's "-T" flag as it's
            # not supported in Alpine
            echo "$content" | install -vm644 /dev/stdin ~/.ssh/config
        fi
        ...
        if [ -n "$files" ]; then
            ...
            # Note: not using install's "-T" flag as it's
            # not supported in Alpine
            echo "$content" | install -vm600 /dev/stdin ~/.ssh/known_hosts
        fi
