...
- `logger -tmytag -p4 'Some warning'`, `echo '<3>Some error' | logger -stmytag --prio-prefix`
...
- `printf '%s\n' '[mygdrive]' 'type = drive' 'client_id = ...' 'client_secret = ' 'scope = drive' 'root_folder_id = ...' 'skip_gdocs = true' > ~/.config/rclone/rclone.conf` (see https://rclone.org/drive/#making-your-own-client-id), `rclone config reconnect mygdrive:`
...
- `podman run --rm --log-driver=none ghcr.io/containers/podlet -i podman run -l io.containers.autoupdate=registry --restart=always --net=pasta:-Itap0,--map-guest-addr,none,--outbound-if4,eth0,--outbound-if6,eth0 -p8080:80 docker.io/library/nginx:latest`
